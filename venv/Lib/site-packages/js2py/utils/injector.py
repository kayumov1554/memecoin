...
# Opcode constants used for comparison and replacement
...
def append_arguments(code_obj, new_locals):
    ...
    for inst in instructions(code_obj):
        ...
        # If it accesses co_varnames or co_names then update its argument.
            ...
            arg = name_translations.get(inst.arg, None)
            if arg is None:
                continue  # или обработайте ошибку другим образом
...
def write_instruction(op, arg):
    ...
    else:  # python 3.6+ uses wordcode instead of bytecode and they already supply all the EXTENDED_ARG ops :)
        if arg is None:
            return [chr(op), 0]
